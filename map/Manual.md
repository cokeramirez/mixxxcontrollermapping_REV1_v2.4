# Pioneer DDJ-REV1 {#pioneer-ddj-rev1}
...
- [Manufacturer's product page](https://www.pioneerdj.com/en/product/controller/ddj-rev1/black/overview/l)
- [Manufacturer's manual](https://downloads.support.alphatheta.com/manuals/DDJ_REV1_DRI1744B_manual/)
...
- [Mapping forum thread](https://mixxx.discourse.group/t/pioneer-ddj-rev1-mapping-update-2-6/32603)
...
| Mode | Behavior |
|------|----------|
| **2.5:** | Scratch Bank on **SCRATCH Bank** pads 1–4 (samples 17–24). |
| **2.6+:** | Same pads on **SCRATCH Bank** control stems (mute). Scratch Bank is available via [Mixxxed Mode](#mixxxed-mode) slot 4 when enabled. Stem/EQ options apply only when stem tracks and stem controls are available. |
...
> The [example setups section](https://manual.mixxx.org/2.6/en/chapters/example_setups#controllers-with-an-integrated-audio-interface) provides more details about the audio configuration in Mixxx.
...
> **See also**  
> The [gain staging documentation](https://manual.mixxx.org/2.6/en/chapters/djing_with_mixxx#setting-your-levels-properly-gain-staging) explains how to set your levels properly when using Mixxx.
...
- **SHIFT** + **PLAY/PAUSE** supports braking profiles (Off,
...
- STEMS v2.6+.
- VU meter options.
- Additional user configuration options (see [User configuration options](#user-configuration-options)).
- Mixxxed Mode (configurable per deck — see [MIXXXED Mode](#mixxxed-mode)).
...
| No. | Control | Function |
|-----|---------|----------|
| 1 | **SHIFT** + **LOAD** | Sort by user-selected configuration. Double press toggles ascending/descending. |
| 2 | **Rotary Selector PUSH** | Push Rotary selector to cycle forward between panels in library. |
| 2 | **SHIFT** + **Rotary Selector PUSH** | Hold shift + Push Rotary selector to cycle backwards between panels in library. |
| 3 | **SHIFT** + **Rotary Selector** | Rotate selector while holding SHIFT to move left or right in the library or open and close the subcrates panel. |

### Deck section

| No. | Control | Function |
|-----|---------|----------|
| 6 | **SYNC** | Temporary beat sync. If sync lock is active, a short press cancels the lock. |
| 6 | **SHIFT** + **SYNC** | Enable sync lock. |
| 7 | **SHIFT** + **PLAY/PAUSE** | Braking disabled: stutter-play. Braking enabled: uses the configured start/stop brake profile (Off / Classic / Slow). |
| 9 | **JOG WHEEL** top / side Vinyl mode: | Top scratches, side bends (or waveform zoom when enabled). CDJ mode: jog uses bend behavior. |

### Mixer section

| No. | Control | Function |
|-----|---------|----------|
| 12 | **(HEADPHONES) CUE** | PFL toggle with AUTO head-mix adjustment. |
| 12 | **MASTER CUE** | Toggles master head-mix blend (AUTO PFL). |
| 13 | **SHIFT** + **CHANNEL FADER** | Channel fader start (must be enabled in Utility mode on the controller). |
| 15 | **SHIFT** + **CROSSFADER** | Crossfader start (must be enabled in Utility mode on the controller). |
...
| No. | Control | Function |
|-----|---------|----------|
| 4 | **LEVEL/DEPTH** | Adjusts the parameter of the enabled effects for FX1 / FX2. |
| 3 | **SHIFT** + **FX 1** | Cycle to the next effect-chain preset after the currently loaded preset (descending order). |
| 4 | **SHIFT** + **FX 2** | Adjust the average BPM up by +0.01 (beat grid lines move closer together). |
| 5 | **SHIFT** + **FX 3** | Adjust the average BPM down by −0.01 (beat grid lines move farther apart). |
| 4 | **FX1**, **FX2**, **FX3** + **ROTARY SELECTOR** | Designate the effect for the selected FX button (descending order). |

### Beat Jump / Roll section

| No. | Control | Function |
|-----|---------|----------|
| 10 | Beat Jump **pads 1–4** | One-shot beat jump / back / size controls. |
| 10 | Beat Jump **pad 5** | Previous track (deck must not be playing). |
| 10 | Beat Jump **pads 6 /** | Hold-to-rewind / hold-to-fast-forward. |
| 10 | Beat Jump **pad 8** | Hold-to-censor (`reverseroll`). |
| 11 | Beat Roll **pads 1–8** | Hold loop roll with per-pad configurable roll sizes and available actions. |

### Sampler section

| No. | Control | Function |
|-----|---------|----------|
| 10 | **SAMPLER PADS 1–16** | Play the loaded sample, or load the selected track when empty. Follows sampler pad layout. |
| 10 | **SHIFT** + **SAMPLER PADS 1–16** | Stop the playing sample, or eject a stopped sample. |
| 10 | **SAMPLER** + **LEVEL/DEPTH** | Sampler gain for samplers 1–16 while held (sampler volume gate). |
...
| No. | Control | Function |
|-----|---------|----------|
| 10 | **SCRATCH BANK pads 1–4** | Load scratch samples from samples **17–20(Deck1/3) 21-24 (Deck2/4).** |
...
| No. | Control | Function |
|-----|---------|----------|
| 10 | **SCRATCH BANK pads 1–4** | Stem mute toggles (voice / melody / bass / drums). |
| 10 | **SCRATCH BANK pads 5–8** | Stem effect toggles (voice / melody / bass / drums). |
| 10 | **SCRATCH BANK pads 1–4** + **LEVEL/DEPTH** | Adjust stem volume / while held parameters while held. |
...
![Stems and stem-effect positions: voice, melody, bass, drums.](../../_static/controllers/Mixxx-250-Hardware-DDJ_REV1-stems-layout.svg)
...
| No. | Control | Function |
|-----|---------|----------|
| 10 | Scratch Bank **SHIFT** + **Pad1** | AutoDJ. |
| 10 | Scratch Bank **SHIFT** + **Pad2** | AutoDJ fade to next. |
| 10 | Scratch Bank **SHIFT** + **Pad3** | Toggle Microphone. |
| 10 | Scratch Bank **SHIFT** + **Pad4** | Toggle Record Mix. |
| 10 | Scratch Bank **SHIFT** + **Pad5** | Key Match. |
| 10 | Scratch Bank **SHIFT** + **Pad6** | Beat Grid. |
| 10 | Scratch Bank **SHIFT** + **Pad7** | Pitch Up. |
| 10 | Scratch Bank **SHIFT** + **Pad8** | Pitch Down. |
...
![Mixxxed Mode slot 4 — Scratch Bank. Scratch samples loaded.](../../_static/controllers/Mixxx-250-Hardware-DDJ_REV1-lights-mode4.svg)

*Mixxxed Mode slot 4: Scratch Bank. Scratch samples loaded.*

| No. | Control | Function |
|-----|---------|----------|
| 0 | Auto Loop Mode **SHIFT** + **←FX2/FX3→** | Cycle Mixxxed Mode (per deck). |
...
| 2 | Auto Loop Mode **SHIFT** + **Pad7** | Cycle Beat Slicer domain 8 / 16 / 32 / 64. Loop size and slice jump size increase accordingly. |
| 2 | Auto Loop Mode **SHIFT** + **Pad8** | Toggle Beat Slicer loop mode. |

| | |
|---|---|
| ![Mixxxed Mode slot 2 — Beat Slicer linear mode pad lighting example.](../../_static/controllers/Mixxx-250-Hardware-DDJ_REV1-lights-mode2-linear.svg) | ![Mixxxed Mode slot 2 — Beat Slicer loop mode pad lighting example.](../../_static/controllers/Mixxx-250-Hardware-DDJ_REV1-lights-mode2-loop.svg) |
...
| | |
|---|---|
| ![Piano Roll Major mode — deck 1 major scale (C example).](../../_static/controllers/Mixxx-250-Hardware-DDJ_REV1-Major-Deck1-MajorScale.svg) | ![Piano Roll Major mode — deck 2 minor scale (C example).](../../_static/controllers/Mixxx-250-Hardware-DDJ_REV1-Major-Deck2-MinorScale.svg) |
...
| | |
|---|---|
| ![Piano Roll Play Through — deck 1 major scale (C example).](../../_static/controllers/Mixxx-250-Hardware-DDJ_REV1-PT-Deck1-MajorScale.svg) | ![Piano Roll Play Through — deck 2 minor scale (C example).](../../_static/controllers/Mixxx-250-Hardware-DDJ_REV1-PT-Deck2-MinorScale.svg) |
| Deck 1 — Major scale | Deck 2 — Minor scale |
| ![Piano Roll Play Through — deck 1 major scale SHIFT row (C example).](../../_static/controllers/Mixxx-250-Hardware-DDJ_REV1-PT-Deck1-MajorScale-SHIFT.svg) | ![Piano Roll Play Through — deck 2 minor scale SHIFT row (C example).](../../_static/controllers/Mixxx-250-Hardware-DDJ_REV1-PT-Deck2-MinorScale-SHIFT.svg) |
...
>   Releasing a piano pad stops playback. Press **PLAY** while holding a piano pad for **continuous
>   play** in that key.
> - On a **playing** deck, pressing a piano pad sets anchor stops playback on release. Press **PLAY** while holding a piano pad for **continuous
>   play** in that key.
> - Press **PLAY** alone to roll from the anchor; the next pad press
>   stops transport first.
> - **SHIFT** + Auto Loop pad select/deselect an existing hotcue as the anchor
...
|----------|----------|---------|
| `PioneerDDJREV1PROD.vinylMode` | Per-deck startup vinyl / CDJ mode. | `true` |
...
| `PioneerDDJREV1PROD.librarySortDefaults` | Sort by any available library option. | `"artist"`, `"bpm"`, `"date added"`, `"key"` |
| `PioneerDDJREV1PROD.bigLibraryShiftPush` | **SHIFT** + **ROTARY SELECTOR PUSH** Maximize and minimize "Big Library". *Replaces library backwards panel movement. Use forward to cycle through.* | `false` |
| `PioneerDDJREV1PROD.beatJumpSize1` … `beatJumpSize8` | Per-pad configuration for beat jump size, includes action override. | `Back/Half/Double/Forward` … `Prev/RWD/FWD/Censor` |
| `PioneerDDJREV1PROD.autoLoopSize1` … `autoLoopSize8` | Per-pad loop sizes for auto loop mode (1/32...-64, halves/doubles). | `1/16` … `8` |
| `PioneerDDJREV1PROD.beatLoopRollsSize1` … `beatLoopRollsSize8` | Per-pad roll sizes for beat roll mode (1/32...-64, halves/doubles). | `1/4` … `32` |
| `PioneerDDJREV1PROD.sZoom` | Use vinyl side jog for waveform zoom. | `false` |
| `PioneerDDJREV1PROD.waveformZoomMode` | Attach waveform zoom to deck Vinyl or CDJ mode when enabled. | `vinyl` |
| `PioneerDDJREV1PROD.vuMeterMode` | VU meter routing: legacy per-deck meters, or stereo split master (left , right). | `per_deck` |
| `PioneerDDJREV1PROD.brakingEnabled` | Enable profile-based **SHIFT** + **PLAY** braking. | `false` |
| `PioneerDDJREV1PROD.brakingStartProfile` | Start profile for **SHIFT** + **PLAY**: `off` / `classic` / `slow`. | `off` |
| `PioneerDDJREV1PROD.brakingStopProfile` | Stop profile for **SHIFT** + **PLAY**: `off` / `classic` / `slow`. | `off` |
